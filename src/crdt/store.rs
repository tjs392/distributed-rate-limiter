...
impl Default for VelocityEntry {
    fn default() -> Self {
        VelocityEntry { ema: 0.0, last_update: Instant::now() }
    }
}
...
pub struct CRDTStore {
    ...
    */
    ...
    velocity:  DashMap<KeyHash, VelocityEntry>,
}
...
impl CRDTStore {
    ...
    pub fn increment(&self, key_hash: KeyHash, epoch: Epoch, node_id: NodeId, hits: u64, tier: u8, velocity_alpha: f64) {
        ...
        // update velocity EMA
        let now = Instant::now();
        let mut vel = self.velocity.entry(key_hash).or_default();
        let dt_ms = now.duration_since(vel.last_update).as_millis() as f64;
        if dt_ms > 0.0 {
            let instant_rate = hits as f64 / dt_ms;
            vel.ema = velocity_alpha * instant_rate + (1.0 - velocity_alpha) * vel.ema;
            vel.last_update = now;
        }
    }

    pub fn velocity(&self, key_hash: KeyHash) -> f64 {
        self.velocity.get(&key_hash)
            .map(|v| v.ema)
            .unwrap_or(0.0)
    }
...
mod tests {
    ...
    fn increment_and_read() {
        ...
        store.increment(KEY, EPOCH, NODE_A, 10, 0, 0.3);
    ...
    fn increment_multiple_nodes() {
        let store = CRDTStore::new();
        store.increment(KEY, EPOCH, NODE_A, 10, 0, 0.3);
        store.increment(KEY, EPOCH, NODE_B, 20, 0, 0.3);
    ...
    fn different_keys_are_independent() {
        let store = CRDTStore::new();
        store.increment(100, EPOCH, NODE_A, 5, 0, 0.3);
        store.increment(200, EPOCH, NODE_A, 15, 0, 0.3);
    ...
    fn different_epochs_are_independent() {
        let store = CRDTStore::new();
        store.increment(KEY, 1, NODE_A, 10, 0, 0.3);
        store.increment(KEY, 2, NODE_A, 20, 0, 0.3);
    ...
    fn increment_marks_dirty() {
        let store = CRDTStore::new();
        store.increment(KEY, EPOCH, NODE_A, 5, 0, 0.3);
    ...
    fn sliding_window_start_of_epoch() {
        let store = CRDTStore::new();
        store.increment(KEY, 41, NODE_A, 100, 0, 0.3);
        store.increment(KEY, 42, NODE_A, 10, 0, 0.3);
    ...
    fn no_previous_epoch_data() {
        let store = CRDTStore::new();
        store.increment(KEY, 42, NODE_A, 50, 0, 0.3);
